...
`volume` is contracts and `trade_count` is prints.

`notional_volume` is **premium paid, in USD**, on BTC, ETH and SOL. On HYPE
it is **underlying notional** (contracts x spot) instead, because that is
what Derive publishes for it. The two differ by roughly the premium ratio,
so do not compare HYPE's figure against the others without converting.
