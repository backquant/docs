...
1. **Chain** - multi-venue options chain for greeks and **coin-unit OI**:
   Deribit, Bybit, OKX, Binance, Derive (when enabled), Thalex, Delta India,
   Delta Global.
2. **Tape (flow only)** - multi-venue aggressor trades for the same coins.
   Size is **coin-normalized** so venues can be summed. See
   [Venue coverage](/concepts/venues) for per-venue ingest.
...
To rebuild strike-level sold/bought maps yourself (TRACE-style), use:

[`GET /v2/tape/flow-by-strike`](/api/v2/tape/flow-by-strike)

That endpoint is cumulative aggressor pressure on **still-open**
contracts - the same primitive the terminal uses for flow positioning.
...
| `/v2/tape/flow-by-strike` | **Aggressor** sold/bought by strike | Options tape |
