...
Profiles default to **`positioning=flow`**, BackQuant's measured dealer model.
Pass `?positioning=std` for textbook call/put OI sign. Forward
(time × price) maps live under
...
- `?exchanges=deribit,bybit,okx,binance,derive,thalex,delta_india,delta` - venue filter
...
<CardGroup cols={2}>
  ...
  <Card title="Flow vs std" icon="scale-balanced" href="/concepts/positioning">
    How profiles and TRACE sign dealer size.
  </Card>
  <Card title="TRACE" icon="chart-area" href="/api/v2/options/greeks/trace">
    Forward time × price projection of dealer greeks.
  </Card>
