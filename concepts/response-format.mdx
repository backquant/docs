...
<ResponseField name="meta" type="object" required>
  ...
  <Expandable title="properties">
    <ResponseField name="meta.version" type="string" required>
      The v2 contract version that served the response - currently `"2.6"`.

      This is `major.minor`, and it **changes**: the minor bumps whenever a
      public default or response contract changes (see the
      [changelog](/api/v2/discovery/changelog)). Treat it as informational,
      or compare the major. Do not assert equality on the full string, or a
      routine release will break your client.
    </ResponseField>
...
Twelve of the 52 v2 endpoints have full Pydantic response schemas in the
OpenAPI spec. Use them with codegen tools (`openapi-python-client`,
...
- `MultiGexLevelsResponse`, `TraceResponse`
- Plus `ChangelogResponse`

The other 40 endpoints are documented with examples and the same
envelope shape.
