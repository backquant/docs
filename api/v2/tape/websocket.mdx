...
`{coin}` is `BTC` or `ETH`. `{venue}` is `deribit`, `bybit`, `okx`, or `binance` (case-insensitive on subscribe; the server normalises to lowercase). See [Tape overview → Venue coverage](/api/v2/tape/overview#venue-coverage) for what each venue ships.
...
<CardGroup cols={2}>
  ...
  <Card title="Rate limits & caps" href="/concepts/rate-limits" icon="gauge" />
</CardGroup>
